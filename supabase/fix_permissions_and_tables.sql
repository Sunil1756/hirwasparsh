...
-- 2. Add Missing Columns to trees Table If Needed
ALTER TABLE public.trees ADD COLUMN IF NOT EXISTS drive_id UUID;
...
-- 3. Create Auxiliary Tables If Not Exists
...
-- 5. Create Clean & Robust RLS Policies
-- TREES
...
CREATE POLICY "Users can update own trees" ON public.trees FOR UPDATE USING (true);
...
DROP POLICY IF EXISTS "Users can insert treebank" ON public.treebank;
CREATE POLICY "Users can insert treebank" ON public.treebank FOR INSERT WITH CHECK (true);
...
DROP POLICY IF EXISTS "Users can insert selfies" ON public.selfies;
CREATE POLICY "Users can insert selfies" ON public.selfies FOR INSERT WITH CHECK (true);
